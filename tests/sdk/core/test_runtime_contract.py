...
# ── Minimal concrete stub for runtime contract verification ──────────────────
...
class TestBaseAgentWithMockRuntime:
    ...
    async def test_compile_is_idempotent(self):
        ...
        await agent.compile()  # second call
        assert agent._compiled is first_ref  # cached reference
    ...
    async def test_checkpoint_and_resume_flow(self):
        ...
        # Simulate HITL pause
        state = AgentState(
            messages=[{"role": "user", "content": "pause"}], status="awaiting_human"
        )
